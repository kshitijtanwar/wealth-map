...
export const AuthServiceTest = () => {
  ...
  const testSessionManagement = async () => {
    try {
      ...
      await authService.signIn(formData.adminEmail, formData.adminPassword)
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ backgroundColor: '#f2f2f2' }}>
              <th style={{ padding: '12px', textAlign: 'left', borderBottom: '1px solid #ddd' }}>Test</th>
              <th style={{ padding: '12px', textAlign: 'left', borderBottom: '1px solid #ddd' }}>Result</th>
              <th style={{ padding: '12px', textAlign: 'left', borderBottom: '1px solid #ddd' }}>Error</th>
          ...
          <tbody>
            {testResults.map((test, index) => (
              <tr key={index} style={{ borderBottom: '1px solid #ddd' }}>
                ...
                <td style={{ padding: '12px', color: test.result.includes('✅') ? 'green' : 'red' }}>
                  {test.result}
                </td>
                <td style={{ padding: '12px', color: 'red' }}>{test.error || '-'}</td>
                <td style={{ padding: '12px' }}>{new Date(test.timestamp).toLocaleTimeString()}</td>
              </tr>
            ))}
          </tbody>
        </table>
